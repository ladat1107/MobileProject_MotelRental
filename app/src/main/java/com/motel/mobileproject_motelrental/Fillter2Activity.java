...
public class Fillter2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String fillter;
        String[] receivedArray = new String[0];
        String[] chipArray = new String[0];

        if (putType == 0) {
            fillter = getIntent().getStringExtra("fillter");
        } else {
            receivedArray = getIntent().getStringArrayExtra("infoFill");
            chipArray = getIntent().getStringArrayExtra("infoChip");
        ...
        if (putType == 0) {
            FillChipFromHome(fillter);
        } else {
            FillChip(fillter, receivedArray, chipArray);
        }
        MenuClick();
    }

    public void FillChipFromHome(String fillter) {
        ...
        FillListFromHome(fillter);
    }

    public void FillChip(String fillter, String[] receivedArray, String[] chipArray) {
        int city = 0;
        int district = 0;
        String txt = "";
        ...
        if (city == 0) txt = "Tất cả khu vực. ";
        else if (district == 0) txt = receivedArray[0];
        else txt = receivedArray[1] + ", " + receivedArray[0];
        binding.edtKhuVuc.setText(txt);

        if (!receivedArray[6].equals("Tất cả giá")) chipItemList.add(new ChipItem(receivedArray[6]));
        if (!receivedArray[7].equals("Tất cả")) chipItemList.add(new ChipItem(receivedArray[7]));

        for (int i = 0; i < chipArray.length; i++) {
            chipItemList.add(new ChipItem(chipArray[i]));
        }

        FillList(fillter, receivedArray, chipArray);
        ChipAdapter adapter = new ChipAdapter(chipItemList);
        adapter.attachToFlowLayout(binding.flowchip);
    }

    public void FillListFromHome(String fillter) {
    ...
    public void FillList(String fillter, String[] receivedArray, String[] chipArray) {
        ...
        if (receivedArray[6].equals("Giá tăng dần")) {
            query = query.orderBy(Constants.KEY_PRICE, Query.Direction.ASCENDING);
        } else if (receivedArray[6].equals("Giá giảm dần")) {
            query = query.orderBy(Constants.KEY_PRICE, Query.Direction.DESCENDING);
        } else {
            query = db.collection(Constants.KEY_COLLECTION_MOTELS);
        }

        handelFilterPage(query, fillter, receivedArray, chipArray, adapterInfo, motelList);
